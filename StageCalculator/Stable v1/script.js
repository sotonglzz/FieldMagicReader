...
function drawRectangle() {
    ...
    // Get the available dimensions (80% of the viewport)
    ...
    const rotatedLargePanelsAlongLength = Math.floor(length / rotatedLargePanelLength);
    const rotatedLargePanelsAlongWidth = Math.floor(width / rotatedLargePanelWidth);
    ...
    const remainingLength = Math.floor(remainingArea / rotatedLargePanelArea);
    ...
    for (let row = 0; row < rotatedLargePanelsAlongLength; row++) {
        for (let col = 0; col < rotatedLargePanelsAlongWidth; col++) {
            if (rotatedLargePanelCount >= remainingLength) {
                break;
            }
